...
# Brief: [Moving Carts in a text file, P2]
...
while len(cart_pos) > 1:
    crash_carts = set()
    cart_pos = cart_pos.sort_values(by=['pos_complex']).reset_index(drop=True)

    for no in cart_pos.index.copy():
        if no in crash_carts:
            continue
        moves += 1
        ...
        if crash:
            crashed_indices = cart_pos[cart_pos['pos_complex'].isin(crash_coords['pos_complex'])].index
            crash_carts.update(crashed_indices)

            crash_df = pd.concat([crash_df, crash_coords]).drop_duplicates(subset=['cart']).reset_index(drop=True)


    cart_pos = cart_pos.drop(index=crash_carts).reset_index(drop=True)
