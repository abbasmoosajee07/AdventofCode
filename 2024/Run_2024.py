...
def create_plot(df, challenge, Year, center_color="#4CAF50", scale='linear'):
    ...
    percentages = [(time / total_mem) * 100 for time in avg_mems]  # Percentage of total memory for each day
    ...
    if scale == 'linear':
        ...
        ax.set_ylim(max(min(avg_times) * 0.9, 1e-2), max(avg_times) * 1.5)  # Adjust for log scale
    ...
    for i, (bar, percentage) in enumerate(zip(bars, percentages)):
        ...
        ax.annotate(f"({file_path}) {line_count} lines \n ({memory:.1f} MB)", 
                    xy=(label_x, label_y),
                    xytext=(label_x + 0.05, label_y + 0.05),
    ...
    ax.minorticks_on()
    ax.grid(visible=True, which='minor', linestyle=':', linewidth=0.5, alpha=0.4)

    ax.legend(loc='upper left', bbox_to_anchor=(0, 1), fontsize=9,
                frameon=True, facecolor='white', edgecolor='black')
    # Add custom information to the legend (using proxy artists)
    custom_lines = [
        plt.Line2D([0], [0], color='white', label=f"Total Time: {total_time:.2f}s"),
        plt.Line2D([0], [0], color='white', label=f"Total Memory: {total_mem:.2f}MB"),
        plt.Line2D([0], [0], color='white', label=f"Scale: {scale.capitalize()}"),
        plt.Line2D([0], [0], color='white', markersize=1),
    ]

    # Existing legend items (e.g., for average, median, max, min)
    legend_items = ax.legend(loc='upper left', bbox_to_anchor=(0, 1), fontsize=9,
                            frameon=True, facecolor='white', edgecolor='black')
...
if __name__ == "__main__":
    ...
    run_2024 = main(Challenge, Year, days_to_run, num_iterations, color_2024)
