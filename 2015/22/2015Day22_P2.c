...
int battle_sim(Wizard *wizard, Boss *boss) {
    ...
    while (1) {
        ...
        wizard->hit_points -= 1;
        sprintf(buffer, "Hard mode: Wizard loses 1 HP at start of turn. HP: %d\n", wizard->hit_points);
        append_to_log(buffer);

        if (wizard->hit_points <= 0) {
            append_to_log("Wizard is dead due to hard mode HP penalty!\n");
            return 0;
        }
...
void save_log_to_file(const char *filename) {
    ...
    if (last_backslash) {
        *last_backslash = '\0';  // Remove the executable name from the path
    }
    ...
    sprintf(full_path, "%s/%s", dirname(exe_path), filename);
#endif

    FILE *file = fopen(full_path, "w");
    if (file != NULL) {
        fputs(battle_log_P2, file);
        fclose(file);
...
int main() {
    ...
    for (int trial = 0; trial < 100000; trial++) {
        ...
        Wizard wizard = {50, 500, 0};  // Wizard starts with 50 HP and 500 Mana
        Boss boss = {58, 9};  // Boss starts with 58 HP and deals 9 damage
    ...
    save_log_to_file("battle_log_P2.txt");  // Save the log to a file
