...
class LogoWrapper
    ...
    def num_spaces_painted
        @painted_spaces.keys.count
    end
...
def read_inputs
    File.readlines(D11_file_path).map(&:strip)[0].split(",").map(&:to_i)
end
...
def main
    ...
    puts "Part 1: #{logo_wrapper.num_spaces_painted}"
end
