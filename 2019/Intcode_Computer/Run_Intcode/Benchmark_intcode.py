...
def perform_benchmark(program, program_name, test_inputs):
    """
    Executes an Intcode program, measures its runtime, and returns results as a DataFrame.
    """
...
result_ackermann = perform_benchmark(ackermann_program, "Ackermann", [3, 6])
...
result_isqrt = perform_benchmark(isqrt_program, "Integer Square Root", 16)
...
result_divmod = perform_benchmark(divmod_program, "Divmod", [1024, 3])
...
benchmarks.to_csv(file_path, sep='\t', index=False)
